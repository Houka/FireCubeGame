...
*/
void EnemyModel::setShipNode(const std::shared_ptr<cugl::Node>& node) {
	if (_shipNode != nullptr) {
...
*/
const std::shared_ptr<cugl::AnimationNode>& EnemyModel::getBurnerStrip(Burner burner) const {
	switch (burner) {
	case Burner::MAIN:
		return _mainBurner;
	case Burner::LEFT:
...
*/
void EnemyModel::setBurnerStrip(Burner burner, const std::shared_ptr<cugl::Texture>& strip) {
	switch (burner) {
	case Burner::MAIN:
		_mainBurner = AnimationNode::alloc(strip, 1, FIRE_FRAMES, FIRE_FRAMES);
		if (_shipNode != nullptr) {
			_mainBurner->setPosition(_shipNode->getContentSize().width / 2.0f,
				_shipNode->getContentSize().height -
				_mainBurner->getContentSize().height / 2.0f);
		...
		break;
	case Burner::LEFT:
		_leftBurner = AnimationNode::alloc(strip, 1, FIRE_FRAMES, FIRE_FRAMES);
		if (_shipNode != nullptr) {
			_leftBurner->setPosition(_shipNode->getContentSize() / 2.0f);
			_shipNode->addChild(_leftBurner);
		}
		break;
	case Burner::RIGHT:
		_rghtBurner = AnimationNode::alloc(strip, 1, FIRE_FRAMES, FIRE_FRAMES);
		if (_shipNode != nullptr) {
			_rghtBurner->setPosition(_shipNode->getContentSize() / 2.0f);
			_shipNode->addChild(_rghtBurner);
		}
...
*/
const std::string& EnemyModel::getBurnerSound(Burner burner) const {
	switch (burner) {
	case Burner::MAIN:
		return _mainSound;
	case Burner::LEFT:
...
*/
void EnemyModel::setBurnerSound(Burner burner, const std::string& key) {
	switch (burner) {
	case Burner::MAIN:
		_mainSound = key;
		break;
...
*/
void EnemyModel::animateBurner(Burner burner, bool on) {
	AnimationNode* node;
	bool*  cycle;
