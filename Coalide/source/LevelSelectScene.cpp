...
*/
bool LevelSelectScene::init(const std::shared_ptr<AssetManager>& assets, InputController input, std::string levelKey) {
...
*/
void LevelSelectScene::activateWorldCollisions() {
...
void LevelSelectScene::updateFriction() {
...
void LevelSelectScene::removeEnemy(std::shared_ptr<EnemyModel> enemy) {
...
void LevelSelectScene::removeObject(std::shared_ptr<ObjectModel> object) {
	// do not attempt to remove a bullet that has already been removed
	if (object->isRemoved()) {
		return;
	}
...
*
*/
void LevelSelectScene::reset(const std::string& file) {
